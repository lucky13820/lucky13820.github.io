function addPriceFormListeners() {
  ...
  document.querySelector('[data-price="99"]').click();
...
const trackSlideChange = (slide) => {
  const slideEventValue = slide.getAttribute("data-slide-event") || "No data-slide-event found";
...
const validateSlide = (swiper, report = false) => {
  ...
  const inputs = Array.from(currentSlide.querySelectorAll("input[required], select[required], textarea[required]"));
  ...
  swiper.el.addEventListener("input", (e) => {
    ...
    if (["INPUT", "SELECT", "TEXTAREA"].includes(targetElement.tagName)) {
      ...
      if (targetElement.id === "height" && targetElement.value !== "") {
        ...
        if (range) {
          const minWeightRequirement = document.querySelector('[data-id="minimum-weight-range"]');
          const minRecommendedWeight = document.querySelector('[data-id="minimum-recommended-weight"]');
      ...
      const weightProgressNote = document.querySelector('[data-id="weight-progress-note"]');
      ...
      const idealWeightNote = document.querySelector('[data-id="ideal-weight-note"]');

      if (targetElement.id === "ideal-weight" && targetElement.value.length > 1) {
      ...
      if (targetElement.getAttribute("name") === "Familiarity" && targetElement.value === "Expert") {
        const infoText = document.getElementById("info-text");
        const additionalText = "OK, we know you're an expert, but just to recap:";
...
const animatePredictionValue = (prediction) => {
  ...
  const predictionDisplays = document.querySelectorAll('[data-id="prediction"]');
...
quizForm.addEventListener("submit", (e) => {
  ...
  const formDataInstance = new FormData(document.getElementById("wf-form-Compounded-Semaglutide"));
  ...
  if (optInCheckbox) {
    const sms_mktg_opt_in = optInCheckbox.checked && phoneInput && phoneInput.value.trim() !== "";
    formDataInstance.set("sms_mktg_opt_in", sms_mktg_opt_in);
    formDataInstance.delete("Opt-In");
  }

  // transform on & off to true & false
  const formData = Array.from(formDataInstance.entries()).reduce((obj, [key, value]) => {
...
async function updatePrice() {
  try {
    ...
    const checkoutPrices = document.querySelectorAll('[data-id="checkout-price"]');
...
function updatePopupPrice(newPrice) {
  ...
  if (elements.length > 0) {
    ...
    elements.forEach((el) => {
      ...
      const updatedText = currentText.replace(dollarAmountRegex, `$${newPrice}`);
...
function getWeight(height, weight) {
  ...
  const finalValue = Math.max(0, weight - (result < minValues[height] ? minValues[height] : result));
...
function formatAsYouType(event) {
  ...
  if (
    (phoneInput.value.length <= 5 && ["deleteContentBackward", "deleteContentForward"].includes(event.inputType)) ||
...
function checkStateSelection() {
  ...
  if (stateInput) {
    ...
    if (selectedState === "louisiana") {
      ...
      document.getElementsByClassName("compounded-popup")[0].style.display = "flex";
...
function initializeBraze() {
  ...
  const config = {
    apiKey: isStaging ? "eb6e2c94-2401-40ea-b4ee-bc75de73d716" : "3e61333b-f1e0-444b-870c-6d0314d8b892",
...
function createBrazeUser(data) {
  ...
  Object.entries(rest).forEach(([key, value]) => user.setCustomUserAttribute(key, value));
