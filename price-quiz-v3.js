...
const alreadyAdvancedHeightSlide = false;
...
function getWeightRange(height) {
  if (Object.prototype.hasOwnProperty.call(weightMap, height)) {
    return {
...
function calculateBMI(height, weight) {
  ...
  const heightFeet = Number.parseInt(heightArray[0]);
  const heightInches = Number.parseInt(heightArray[1].replace(" ", ""));
...
function healthyWeightRange(heightString) {
  ...
  const heightFeet = Number.parseInt(matches[1], 10);
  const heightInches = Number.parseInt(matches[2], 10);
...
function addPriceFormListeners() {
  const form = document.querySelector("#wf-form-Price-Options-Form");
...
function toggleSlide(swiper, shouldRemove, slideEventValue) {
  if (shouldRemove) {
    ...
    if (!Object.prototype.hasOwnProperty.call(removedSlides, slideEventValue)) {
      swiper.slides.forEach((slide, index) => {
        if (slide.getAttribute("data-slide-event") === slideEventValue) {
          const slideToRemove = slide.cloneNode(true);
    ...
    if (Object.prototype.hasOwnProperty.call(removedSlides, slideEventValue)) {
      const slideData = removedSlides[slideEventValue];
