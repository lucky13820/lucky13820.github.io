...
async function handleSubmit(e) {
  ...
  const firstName = document.querySelector("#first-name")?.value?.replace(/^\s+|\s+$/g, '');
  const lastName = document.querySelector("#second-name")?.value?.replace(/^\s+|\s+$/g, '');  // Only trim start/end
  ...
  // Check if last name is a single letter
